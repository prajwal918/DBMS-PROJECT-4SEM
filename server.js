...
app.post('/api/requests', asyncHandler(async (req, res) => {
    ...
    // 1. Save the request in the Requests table
    await pool.request()
    ...
    // 2. Reward the user with Green Points
    await pool.request()
        .input('userId', sql.Int, userId)
        .query('UPDATE Users SET green_points = green_points + 10 WHERE id = @userId');

    // 3. Create or update a Bin so the Driver sees it immediately!
    let locationName = type === 'Special Pickup' ? 'Pickup: ' + details.substring(0, 25) : details.replace('Location: ', '');
    let binLevel = type === 'Special Pickup' ? 'Normal' : 'Overflowing';
    
    await pool.request()
        .input('location', sql.VarChar, locationName)
        .input('binLevel', sql.VarChar, binLevel)
        .query(`
            IF EXISTS (SELECT 1 FROM Bins WHERE location = @location)
                UPDATE Bins SET level = @binLevel, status = 'Pending' WHERE location = @location
            ELSE
                INSERT INTO Bins (location, zone, status, level) VALUES (@location, 'Citizen Request', 'Pending', @binLevel)
        `);
